    ...
    let callback = Callback::from(move |e: InputEvent| {
        ...
        if let Some(experience) = next.get_mut(idx) {
            match cons(input.value()) {
                ExperienceField::Employer(employer) => experience.employer = employer,
                ExperienceField::Team(team) => experience.team = Some(team),
                ExperienceField::Title(title) => experience.title = title,
                ExperienceField::Dates(dates) => experience.dates = dates,
                ExperienceField::Location(location) => experience.location = location,
            }
            on_change.emit(next);
        }
    });
            ...
            html! {
                ...
                <div class="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-1 mt-0">
                    <div class="flex gap-x-1 min-w-0">
                        ...
                        <span> {" | "} </span>
                        <span> {experience.team.clone()} </span>
                        <span> {" | "} </span>
                        <span> {experience.title.clone()} </span>
                    </div>
                    <div class="flex gap-x-1 whitespace-nowrap">
                        <span> {experience.dates.clone()} </span>
                        <span> {" | "} </span>
                        <span> {experience.location.clone()} </span>
                    </div>
                </div>
                <div>
                    {parsed}
